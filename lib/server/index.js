...
WebdriverServer.prototype.detectDevice = function() {
  ...
  let browserName = this.desiredCapabilities.browserName.toLowerCase();

  switch (browserName) {
    case 'chrome':
      var Chrome = installer('macaca-chrome');
      return new Chrome(this.desiredCapabilities);
      break;
    case 'electron':
      var Electron = installer('macaca-electron');
      return new Electron(this.desiredCapabilities);
      break;
  }

  throw new errors.NoSuchDriver(`${platformName} or ${browserName} is not supported!`);
};
